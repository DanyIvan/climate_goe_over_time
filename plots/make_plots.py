...
def parameter_space():
    ...
    '''
    # read model output
    output = pd.read_csv('reduced_model_output/steady_states.csv')
...
def results_with_proxies(linear_increase=False):
    '''
    ...
    fig, axs = plt.subplots(7, 1, figsize=(a4_x * 0.75, a4_y * 0.55),
        sharex=True, gridspec_kw={'height_ratios': [1, 0.5, 1, 1, 1, 1, 1]})
    ...
    if not linear_increase:
        plt.scatter(profile_points_age, profile_points_temp, marker='x',
                    color='k', s=10, zorder=10, linewidths=0.5)
    ...
    se1 = matplotlib.patches.Rectangle((glc_t3 - 12,0.65), -6, 0.15, color='#de626d')
    se2 = matplotlib.patches.Rectangle((glc_t4 - 20,0.65), -150, 0.15, color='#de626d')
    mq1 = matplotlib.patches.Rectangle((glc_t3 - 12,0.25), -6, 0.15, color='#de626d')
    mq2 = matplotlib.patches.Rectangle((glc_t4 - 20,0.25), -6, 0.15, color='#de626d')
    for i in range(6):
        se = matplotlib.patches.Rectangle((2075 -12*i,0.65), -6, 0.15, color='#de626d')
        axs[1].add_patch(se)
    axs[1].add_patch(se1)
    axs[1].add_patch(se2)
    axs[1].add_patch(mq1)
    ...
    # # legend
    if not linear_increase:
        axs[3].arrow(2100, 5e-5, 0, -2.5e-5, head_width=4, 
            width=0, head_length=0.8e-5, fc='k', ec='k')
        axs[3].hlines(y=5e-5, xmin=2100 - 5, xmax=2100 + 5, linewidth=0.8,
            color='k')
        plt.text(2100 -10, 2.5e-5, 'MIF-S constraints', fontsize=5)
        axs[3].arrow(2100, 1e-4, 0, 1e-4, head_width=4, 
            width=0, head_length=1.1e-4, fc='#A42A04', ec='#A42A04')
        ...
        axs[3].arrow(2100, 3e-7, 0, 4e-7, head_width=4, 
            width=0, head_length=5e-7, fc='#A42A04', ec='#A42A04')
    ...
    plt.ylabel(r'Column ($/cm^2$)', fontsize='6')
    plt.ylim([1e9, 1e19])
...
parameter_space()
results_with_proxies(linear_increase=False)
results_with_proxies(linear_increase=True)
profiles()